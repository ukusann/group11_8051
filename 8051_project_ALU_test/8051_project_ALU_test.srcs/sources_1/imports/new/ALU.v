...
reg [`MSB_8:0] A_out_temp;


initial begin
    op_alu = (alu_flag && valid_insr_en)? op: 4'h0; 
end
...
addSub adder     (op, ci, a, b, result_addsub, co, x, underf, overf);
RlRr shift       (op, a, result_rlrr);
xorAndOr logic   (op, a, b, result_logic, p);
cpl cpl          (op, a, result_cpl, cpl_b, a[0]);
...
always @(posedge clk) begin
    if (rst) begin
        ...
        op_alu <= (alu_flag && valid_insr_en)? op: 4'h0; 
        A_out_temp <= (op_alu == `ADD || op_alu == `INC || op_alu == `SUBB || op_alu == `DEC)? result_addsub :
